...
def test_two_player_action_history_preflop():
    env = make_wrapped_env(n_players=2,
                           starting_stack_sizes=[DEFAULT_STARTING_STACK_SIZE for _ in
                                                 range(2)])
    obs, _, _, _ = env.reset()
...
def test_player_cards():
    for n_players in range(2, 7):
        ...
        bits = obs[start:end]
        assert sum(
            [bit for bit in bits if bit == 1]) == 4  # (suit, rank) one-hot for two cards
        ...
        vec = env._vectorizer
        assert sum(
            [bit for bit in bits if bit == 1]) == 0  # other players cards are hidden


def test_player_cards_seer():
    for n_players in range(2, 7):
        starting_stack_sizes = [DEFAULT_STARTING_STACK_SIZE for _ in range(n_players)]
        env = make_wrapped_env(n_players, starting_stack_sizes,
                               AgentObservationType.SEER)
